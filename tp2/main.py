...
def create_similarity_matrix(tracks: list[tuple], detections: list[tuple]):
    ...
    for t in range(num_tracks):
        for d in range(num_dets):
            iou_score = compute_iou(tracks[t], detections[d])
            similarity_matrix[t, d] = iou_score
            
    return similarity_matrix

# Load detections
detections_dict = load_det("ADL-Rundle-6/det/Yolov5l/det.txt", separator=" ")
...
for frame in tqdm(frames):
    ...
    if active_tracks:
        ...
        track_positions = [t.position for t in active_tracks]
        
        # Create similarity matrix
        sim_matrix = create_similarity_matrix(track_positions, det_positions)
        
        # Cost matrix for Hungarian (minimize 1 - IoU)
        ...
        for track_idx, det_idx in matches:
            active_tracks[track_idx].position = det_positions[det_idx]
            active_tracks[track_idx].missed = 0
        
        # Handle unmatched tracks: increment missed, remove if exceeds max
        new_active_tracks = []
